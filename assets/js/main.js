(function(){
  console.log('RG Build-a-Box loaded');
  ...
  function renderGallery(){
    ...
    list.forEach(item => {
      ...
      img.alt = item.color || '';
      img.onerror = () => {
        console.warn('gallery img 404:', src);
      ...
      if (item.tier === 'core'){
        ...
        b.className = 'badge core';
        b.textContent = 'Fan Favorites';
        badges.appendChild(b);
      } else if (item.tier === 'seasonal'){
        const b = document.createElement('span');
        b.className = 'badge seasonal';
        b.textContent = 'Seasonal Colors';
        badges.appendChild(b);
      }
  ...
  BTN_CART.addEventListener('click', () => {
    if (slots.filter(Boolean).length !== MAX_SLOTS){
      alert('Please complete all 9 slots before adding your box to cart.');
      return;
    }
